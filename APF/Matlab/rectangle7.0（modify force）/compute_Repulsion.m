...
for j = 1:OBJ_NUM
    ...
    Frer_X_one(j) = sum(Frerx) ;    %叠加单个障碍物所有边界斥力在X方向的分量
    Frer_Y_one(j) = sum(Frery) ;    %叠加单个障碍物所有边界斥力在Y方向的分量
end
Frer_X = sum(Frer_X_one) ;    %叠加所有障碍物斥力在X方向的分量
Frer_Y = sum(Frer_Y_one) ;    %叠加所有障碍物斥力在Y方向的分量