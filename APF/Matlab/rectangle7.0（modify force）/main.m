...
START = [70 55];        %起点   
GOAL = [20 55];   %终点
Model = 1 ;            %选择搜索路径模式（1-单向：从起点到终点；2-单向：从终点到起点：3-双向：从起点开始第一个点；3-双向：从终点开始第一个点）
MAP = 0;              %选择地图
if MAP == 0
    OBJ_NUM = 1;
    OBJ = [30 30;50 30 ;50 70;30 70];
elseif MAP == 1
    OBJ_NUM = 8;       %障碍物个数
...
for i = 1:MAX_CYCLE
    ...
    [Frer_X ,Frer_Y] = compute_Repulsion(cur_Position ,goal_Position ,OBJ ,OBJ_NUM ,REP , INF_DISTANCE ,A ,theta_Goal);     %计算出各个障碍物在x方向的斥力与在y方向的斥力
    ...
    while(whether_inside_OBJ(next_Position , OBJ))                   %若计算出的下一点坐标在障碍物内，则增大斥力重复计算直到下一点不在障碍物内
        REP_ASS = REP_ASS * 2;
        [Frer_X ,Frer_Y] = compute_Repulsion(cur_Position ,goal_Position ,OBJ ,OBJ_NUM ,REP_ASS , INF_DISTANCE ,A ,theta_Goal);
...
if success_Flag == 1
   ...
   plot([cur_Position(1) goal_Position(1)],[cur_Position(2) goal_Position(2)],'-r','LineWidth',2);
end
