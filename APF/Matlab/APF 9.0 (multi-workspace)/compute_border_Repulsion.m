function [Frer_X,Frer_Y]=compute_border_Repulsion(cur_position ,goal_position ,border ,REP,INF_DISTANCE , A ,theta_Goal)     
    %计算的边界斥力(当前点，目标点，边界，斥力计算参数，障碍影响距离，辅助参数 ,引力角度)
    %与计算矩形障碍物斥力相似，参考compute_rec_Repulsion
    ...
    hLine = rLine;  %当前点与四条边界的垂直距离
    line_s = border;    %线段起点
    line_e = [(border(2 : 4 , 1 : 2)) ; (border(1 : 1 , 1 : 2))];   %线段终点
    for i = 1:4    %四条线
        if rLine(i) > INF_DISTANCE    %如果每个障碍和路径的距离大于障碍影响距离，斥力为0
            ...
            Frer1_temp_X = Frer1_temp_X2 - Frer1_temp_X1;                          %以线段正方向（s到e）为x正方向时，单条线段斥力Frer1在x方向分量
            ...
            Frer1_temp_Y = Frer1_temp_Y2 - Frer1_temp_Y1 ;                         %以线段正方向（s到e）为x正方向时，单条线段斥力Frer1在y方向分量
