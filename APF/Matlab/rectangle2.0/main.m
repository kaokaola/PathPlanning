%变量初始化
OBJ_NUM = 1;    %障碍物个数
MAX_CYCLE = 2000000;    %循环次数
ATT = 150.0;     %计算引力时增益系数
REP = 5;      %计算斥力时增益系数
A = 0.5;    %计算斥力时辅助参数
INF_DISTANCE = 2.5;     %障碍影响距离，当障碍和车的距离大于这个距离时，斥力为0，即不受该障碍的影响
L = 0.1;    %步长
OBJ = [4 0; 6 0 ;6 2 ; 4 2];   %障碍物
%OBJ = [4 4; 6 4 ;6 6 ; 4 6];
START_AND_GOAL = [0 0; 10 10];   %起终点
...
for i = 1:MAX_CYCLE
    ...
    [Fata_X ,Fata_Y] = compute_Attract(cur_Position ,goal_Position ,ATT ,theta_Goal);       %计算出x和y方向的引力
        [Frer_X(i) ,Frer_Y(i)] = compute_Repulsion(cur_Position ,goal_Position ,OBJ ,REP , INF_DISTANCE ,A);  %计算出各个障碍物在x方向的引力和与在y方向的引力和
...
X = PATH(:,1);      %路径
Y = PATH(:,2);
plot( START_AND_GOAL(1,1), START_AND_GOAL(1,2) ,'v', START_AND_GOAL(2,1),START_AND_GOAL(2,2), 'ms' , X, Y, '.r');