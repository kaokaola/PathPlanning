...
L = 0.1;    %步长
OBJ = [1 1.2;3 2.5;4 4.5;3 6;6 2;5.5 5.5;8 8.5];   %障碍物
START_AND_GOAL = [9 9;1.0 0.5];   %起终点
...
for i = 1:MAX_CYCLE
    ...
    theta_Obj = compute_obj_Angle(cur_Position, OBJ ,OBJ_NUM);      %计算出起点到障碍物连线与X正方向的角度，即斥力角度，逆时针为正方向
    [Fata_X ,Fata_Y] = compute_Attract(cur_Position ,goal_Position ,ATT ,theta_Goal);       %计算出x和y方向的引力
    [Frer_X ,Frer_Y] = compute_Repulsion(cur_Position ,goal_Position ,OBJ ,REP ,theta_Goal ,theta_Obj ,OBJ_NUM , INF_DISTANCE ,A);  %计算出各个障碍物在x方向的引力和与在y方向的引力和
    ...
    if ((cur_Position(1) - goal_Position(1)) > 0) & ((cur_Position(2) - goal_Position(2)) > 0)      %到达终点附近
        i = i + 1;
